...
- 自动注入项目设定、角色信息、世界状态、相关记忆作为上下文
- AI 自动追踪并更新世界状态变量（主线进度、关键物品、关系变化等）
- 支持置顶重要信息，作为长期记忆使用
...
- ✨ 世界状态自动提取：AI 在对话中自动追踪并更新世界状态变量（主线进度、关键物品、关系变化等）
- ✨ 记忆注入对话上下文：对话时自动检索相关记忆，保持故事连贯性
- ✨ 记忆提取鲁棒性优化：重写提取 prompt，支持三级解析策略，适配本地模型
- 🐛 消息删除级联清理：删除消息时自动清理关联记忆
- 🐛 世界状态标记对用户不可见：内部提取标记从界面和存储中剥离
- 🐛 修复流式光标注入无限循环（MutationObserver 自触发）
- 🐛 修复多字节字符（中文）在流式输出末尾被截断的问题
- 🐛 修复世界状态标记泄露到编辑框、置顶面板和对话上下文
- 🐛 修复记忆提取中 JSON 解析和中文过滤的边界情况
- 🐎 对话 API 数据库查询并行化，减少响应延迟
- 🐎 世界状态写入并行化，提升提取性能
